...
abstract class Identifiable extends ClassProperties
{
    ...
    public static function getIdentifierName():string
    {
        ...
        foreach($refl->getProperties() as $property) {
            if(!empty(VirtualAttribute::getAttributes($property, Identifier::class, ReflectionAttribute::IS_INSTANCEOF))) {
                if(is_null($identifier)) {
                    $identifier = $property;
                } else {
                    throw new \LogicException("Duplicate identifier for ".static::class.": found ".$identifier->getName()." and ".$property->getName());
                }
            }
        }
        if(is_null($identifier)) {
        ...
        if(!self::checkValidIdentifier($identifier)) {
            throw new \LogicException("Invalid identifier ".$identifier->getName()." for ".static::class.": Property should be one or more of: int, string, or Identifiable");
        }
     ...
     * @return static|null
     */
    public abstract static function getFromIdentifier($identifier):?self;
    /**
     * @param int|string|Identifiable $identifier
     * @return static
     */
    public static function getFromIdentifierForced($identifier):self
    {
        ...
        if(is_null($result)) {
            if($identifier instanceof Identifiable) {
                throw new \RuntimeException("Could not find object ".static::class.":".$identifier->getIdentifier());
            } else {
